...
async function effectiveAccess(page: Page): Promise<{ features: string[]; permissions: string[] }> {
  return page.evaluate(async () => {
    ...
    if (!response.ok) throw new Error(`effective access request failed: ${response.status}`)
    return response.json()
  })
}

test('ユーザー管理を中心にGroupType・Group・所属・外部ID・所属変更を管理できる', async ({ page }) => {
  test.setTimeout(90000)
  await loginAs(page, SCENARIO_USERS.admin)
  await page.goto('/admin/access-control')
  await expect(page.getByRole('heading', { name: 'ユーザー・グループ・アクセス管理' })).toBeVisible({ timeout: 15000 })
  ...
  await expect(groupCard.getByText(`${groupTypeName} (${groupTypeCode})`, { exact: false })).toBeVisible()

  await groupCard.getByLabel('グループ種別').selectOption({ label: groupTypeName })
  ...
  await membershipCard.getByLabel('ユーザー').selectOption({ label: SCENARIO_USERS.monthlyEmployee })
  await membershipCard.getByLabel('グループ').first().selectOption({ label: groupName })
  ...
  await identityCard.getByLabel('ユーザー').selectOption({ label: SCENARIO_USERS.monthlyEmployee })
  ...
  await expect(identityCard.getByText(`${SCENARIO_USERS.monthlyEmployee}: E2E_HR / E2E-HR-MONTHLY-001`)).toBeVisible()
  ...
  await changeCard.getByLabel('ユーザー').selectOption({ label: SCENARIO_USERS.punchEmployee })
  ...
  const draftRow = changeCard.getByRole('row').filter({ hasText: SCENARIO_USERS.punchEmployee }).filter({ hasText: 'draft' })
  await expect(draftRow).toBeVisible()
  await draftRow.getByRole('button', { name: '取消' }).click()
  await expect(changeCard.getByRole('row').filter({ hasText: SCENARIO_USERS.punchEmployee }).filter({ hasText: 'cancelled' })).toBeVisible()
...
test('GroupへのFeature・Role付与と個別停止が有効アクセスへ即時反映される', async ({ browser }) => {
  ...
  try {
    ...
    await membershipCard.getByLabel('グループ').nth(1).selectOption({ label: groupName })
    ...
    await roleCard.getByRole('textbox', { name: 'Role名', exact: true }).fill(roleName)
    await roleCard.getByRole('button', { name: 'Role追加' }).click()
    await expect(roleCard.getByText(`${roleName} (${roleCode})`, { exact: false })).toBeVisible()

    await roleCard.locator('select').last().selectOption({ label: roleName })
    await roleCard.locator('fieldset').filter({ hasText: 'system_settings' }).getByRole('checkbox', { name: 'read' }).check()
    await roleCard.getByRole('button', { name: '保存', exact: true }).click()

    await roleCard.locator('select').nth(2).selectOption('group')
    await roleCard.getByLabel('グループ').selectOption({ label: groupName })
    ...
    await expect(roleCard.getByText(new RegExp(`${roleName} / group / global`))).toBeVisible()

    await expect.poll(async () => effectiveAccess(userPage)).toMatchObject({
    ...
    await suspensionCard.getByLabel('ユーザー').selectOption({ label: SCENARIO_USERS.monthlyEmployee })
    await suspensionCard.getByLabel('Feature').selectOption({ label: '管理' })
    await suspensionCard.getByPlaceholder('停止理由').fill('E2E個別停止')
    await suspensionCard.getByRole('button', { name: '停止', exact: true }).click()
    const suspension = suspensionCard.locator('span').filter({ hasText: 'E2E個別停止' })
    await expect(suspension).toBeVisible()
    await expect.poll(async () => effectiveAccess(userPage)).not.toMatchObject({
      features: expect.arrayContaining(['administration']),
    })
...
test('外部HR CSVの差分確認と取込を画面から実行できる', async ({ page }) => {
  ...
  for (const field of ['display_name', 'email']) {
    const fieldRow = identityCard.locator('div.rounded.border.p-2').filter({ hasText: field })
    await Promise.all([
      page.waitForResponse((response) => response.url().includes(`/field-authorities/${field}`) && response.ok()),
  ...
  await csvCard.locator('input[type="file"]').setInputFiles({
    ...
    mimeType: 'text/csv',
    buffer: Buffer.from('external_subject_id,display_name,email\nE2E-HR-NEW-001,E2E 外部社員,e2e-external@example.com\n'),
  })
  await csvCard.getByRole('button', { name: '差分確認' }).click()
  await expect(csvCard.getByText('全1件 / 新規1件 / 変更1件')).toBeVisible({ timeout: 20000 })
  await expect(csvCard.getByRole('row').filter({ hasText: 'E2E-HR-NEW-001' })).toContainText('新規')
  await csvCard.getByRole('button', { name: '確認した差分を適用' }).click()
  await expect(identityCard.getByText('E2E 外部社員: EXTERNAL_HR / E2E-HR-NEW-001')).toBeVisible()

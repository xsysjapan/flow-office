...
`npm run test:e2e`(内部的には`playwright test`)は、テスト本体の実行前に
`globalSetup`(`global-setup.ts`)が`POST /dev/reset-database`を1回呼び、開発DBを
`migrate:fresh --seed` + `ScenarioSeeder`の状態へ自動的にリセットする
(`backend/app/Http/Controllers/Api/DevDatabaseResetController.php`参照)。そのため
**シナリオ用マスタデータの投入を手動で行う必要はない**(backend・mock-oidc・frontendが
起動してさえいればよい)。何度実行しても常に同じ初期状態から始まるため、前回実行分の
日次実績・有給付与・承認済み/締め済みステータスの蓄積や、それによる非冪等な失敗を
気にしなくてよい。

1. `docker compose up` (または devcontainer) で backend・frontend・mock-oidc を起動する。
2. E2Eテストを実行する: `cd frontend && npm run test:e2e`
   (実行開始時に自動でDBがリセットされる。手動で`ScenarioSeeder`を実行する必要はない)
...
1. backend: `composer install` → `cp .env.example .env && php artisan key:generate` →
   ...
   `POST /dev/reset-database`もこのフラグがtrueの時のみ有効になる) →
   `php artisan migrate --seed` → `php artisan serve`(ポート8000)。
   (`db:seed --class=ScenarioSeeder`を手動実行しておく必要はない。E2E実行時の
   `globalSetup`が`migrate:fresh --seed`+`ScenarioSeeder`を自動で行うため。
   backendを起動した直後にAPIの疎通だけ手動確認したい場合は、従来どおり
   `php artisan db:seed --class=ScenarioSeeder`を実行しても問題ない
...
- `playwright.config.ts` — 設定(baseURLはfrontend dev server、`globalSetup`で
  `global-setup.ts`を指定)
- `global-setup.ts` — テスト実行開始時に1回だけ`POST /dev/reset-database`を呼び、
  開発DBを`migrate:fresh --seed` + `ScenarioSeeder`の状態にリセットする
...
- `scenario-08-fiscal-year-cycle.spec.ts` — 同ドキュメント「シナリオ6: 通年運用
  シミュレーション(1年間)」に対応。専用カレンダー・総務担当者(中村恵)で
...
- その他(§5-19): 承認者=申請者本人のスキップ承認(汎用申請・有給・月次勤怠それぞれで
  提出と同時に「承認済み」まで確定すること)。「承認不要」システム設定時に承認者選択欄が
  表示されないことは、system_settingsがE2Eの共有DBを書き換えるグローバル設定のため
  E2Eでは確認せず、各ドメインのフロントエンド単体テストでカバーする
...
- シナリオ6(通年運用シミュレーション): 専用カレンダー(2026-04〜2027-03、GW・お盆・
  年末年始を会社休日クラスタとして設定)・総務担当者(中村恵)で12か月連続の

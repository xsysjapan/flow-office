...
export async function pickDate(
  ...
  const caption = dialog.locator('button').filter({ hasText: /^\d{4}年\d{1,2}月$/ })
  await caption.click()
  const visibleYear = Number((await dialog.getByText(/^\d{4}年$/).textContent())?.replace('年', ''))
  const direction = year >= visibleYear ? '翌年へ' : '前年へ'
  for (let current = visibleYear; current !== year; current += year >= visibleYear ? 1 : -1) {
    await dialog.getByRole('button', { name: direction }).click()
  }
  await dialog.getByRole('option', { name: `${year}年${month}月` }).click()
  await dialog.getByRole('button', { name: new RegExp(`^${year}年${month}月${day}日`) }).click()
}

/** YearMonthPickerで任意の年月を選択する。 */
export async function pickYearMonth(page: Page, label: string, yearMonth: string): Promise<void> {
...
export async function pickTime(
  ...
  await root.getByLabel(label, { exact: options.exact }).click()
  const dialog = page.getByRole('dialog').last()
  await dialog.getByRole('listbox', { name: '時' }).getByRole('option', { name: hour, exact: true }).click()
  await dialog.getByRole('listbox', { name: '分' }).getByRole('option', { name: minute, exact: true }).click()
}

...
test('1か月単位変形労働時間制: 所定を超えて設定した日は、所定を超えた分のみ法定時間外になる', async ({ browser }) => {
  ...
  try {
    ...
    expect(detail.calculation?.work_minutes).toBe(660)
    expect(detail.calculation?.statutory_excess_overtime_minutes).toBe(60)
...
test('裁量労働制: 雇用区分「正社員」と組み合わせ、打刻せずにみなし時間で給与計算される', async ({ browser }) => {
  ...
  try {
    ...
    expect(detail.calculation?.work_minutes).toBe(0)
    expect(detail.calculation?.deemed_work_minutes).toBe(540)
    expect(detail.calculation?.payroll_work_minutes).toBe(540)
    expect(detail.calculation?.statutory_excess_overtime_minutes).toBe(60) // みなし9時間のうち8時間超の1時間
  } finally {
    await close()
  }
})
...
test('管理監督者: 長時間勤務でも残業として計上されない', async ({ browser }) => {
  ...
  try {
    ...
    // 09:00〜21:00(休憩1時間、労働時間11時間)という長時間勤務でも、管理監督者は労働時間規定の
    ...
    expect(detail.calculation?.statutory_excess_overtime_minutes).toBe(0)
    expect(detail.calculation?.statutory_within_overtime_minutes).toBe(0)
  } finally {
    await close()
  }
})

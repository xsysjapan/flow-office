...
test('§5-1: 承認差し戻し→再申請', async ({ browser }) => {
  ...
  try {
    ...
    await expect(approverPage.getByText('差戻し')).toBeVisible()

    // 申請者が差戻しコメントを履歴で確認し、再提出する。
    await applicantPage.reload()
    await expect(applicantPage.locator('.fo-badge', { hasText: '差戻し' })).toBeVisible()
    await applicantPage.getByRole('button', { name: '提出する' }).click()
    await expect(applicantPage.getByText('提出済み')).toBeVisible()
...
test('§5-2: 申請取消(提出後)', async ({ page }) => {
  ...
  await expect(page.locator('.fo-badge', { hasText: '取消' })).toBeVisible()
})
...
test('§5-6+7: ロール変更が即座に反映され、監査ログに記録される', async ({ page }) => {
  ...
  try {
    ...
    // 対象社員は現状employeeロールのみのため、有給ルール画面へのリンクがナビゲーションに
    // 出ないことを確認する。
    await page.reload()
    await expect(page.getByRole('link', { name: '有給ルール' })).toHaveCount(0)
  ...
  await page.reload()
  await expect(page.getByRole('link', { name: '有給ルール' })).toBeVisible()
})

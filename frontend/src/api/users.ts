...
export interface UserFilters { group_id?:string; group_type_id?:number; external_unlinked?:boolean; external_hr?:boolean; account_status?:string }
export function fetchUsers(query?: string, perPage?: number, filters:UserFilters={}): Promise<Paginated<User>> {
  return apiFetch('/users', { query: { q: query, per_page: perPage, ...filters } })
}
...
export function updateUser(id: string, input: Partial<UserProfileInput>): Promise<User> {

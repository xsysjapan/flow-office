...
import {
  ...
  useSpecialLeaveGrantRuleTargetUsers,
...
import { useUpdateSpecialLeaveAutoGrantEnabled } from '../../hooks/useUsers'
...
function SpecialLeaveGrantRuleTargetUsersSection({ ruleId }: { ruleId: number }) {
  const { data: targetUsers, isLoading, error } = useSpecialLeaveGrantRuleTargetUsers(ruleId)
  const updateAutoGrantEnabled = useUpdateSpecialLeaveAutoGrantEnabled()
  ...
  const handleToggle = (userId: string, enabled: boolean) => {
    updateAutoGrantEnabled.mutate(
      ...
      {
        onSuccess: () => {
          void queryClient.invalidateQueries({ queryKey: ['special-leave', 'grant-rules', ruleId, 'target-users'] })
        },
      },
    )
  }
  ...
  return (
    <div className="mt-3 rounded-md border border-border p-3">
      ...
      <FormField label="社員名で絞り込み" htmlFor={`special-leave-rule-${ruleId}-target-user-filter`}>
        <Input
          id={`special-leave-rule-${ruleId}-target-user-filter`}
          value={nameFilter}
              ...
              <tr key={user.id} className="border-b border-border last:border-0">
                ...
                <td className="py-1.5 pr-3">
                  <label className="flex items-center gap-2">
                    <Checkbox
                      checked={user.special_leave_auto_grant_enabled}
                      onCheckedChange={(checked) => handleToggle(user.id, checked === true)}
                      aria-label={`${user.name}の特別休暇自動付与`}
                    />
                    {!user.special_leave_auto_grant_enabled && <Badge tone="warning">自動付与:無効</Badge>}
                  </label>
                </td>
              </tr>
            ))}
        ...
        <ul className="mb-5 divide-y divide-border">
          {(rules ?? []).map((rule) => (
            <li key={rule.id} className="py-3">
              ...
              {expandedRuleIds.has(rule.id) && <SpecialLeaveGrantRuleTargetUsersSection ruleId={rule.id} />}
            </li>
          ))}
        </ul>
      )}

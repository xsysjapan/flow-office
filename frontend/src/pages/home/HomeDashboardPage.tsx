...
import { ArrowRight, CheckCircle2, ClipboardList, Inbox, type LucideIcon } from 'lucide-react'
import { Card } from '../../components/Card/Card'
import { LoadingState } from '../../components/LoadingState/LoadingState'
import { TodayAttendancePanel } from '../../components/TodayAttendancePanel/TodayAttendancePanel'
 ...
 * 「今日の勤怠」は旧`/attendance`(TodayAttendancePage)の実体である`TodayAttendancePanel`を
 * そのまま組み込み、単なるサマリーカードではなく実際の打刻・当日勤怠編集をここで行える
 * ようにする(独立ページは廃止し、`/attendance`はこのホームへリダイレクトする)。
 * 申請センター統合APIは別タスクで新設中のため、それ以外のカードでは既存の個別カウントで
 * 代用する(対象外: 申請センター統合画面自体)。
 */
export function HomeDashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      {canSeeAttendance && <TodayAttendancePanel />}

...
function CompensatoryLeaveRequestForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (successCount === targetDates.length) {
      resetForm()
      setSuccessMessage(`${successCount}日分の代休申請を送信しました。`)
    }
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <FormField label="対象日の指定方法" htmlFor="compensatory-leave-target-mode">
          <NativeSelect
            id="compensatory-leave-target-mode"
            value={targetMode}
        ...
        {targetMode === 'dates' ? (
          <FormField label="対象日" htmlFor="compensatory-leave-target-date" required>
            <div className="flex flex-col gap-2">
              <DatePicker id="compensatory-leave-target-date" value={undefined} onChange={addDate} />
          ...
          <FormField label="対象日(期間)" htmlFor="compensatory-leave-target-range" required>
            <DateRangePicker id="compensatory-leave-target-range" value={range} onChange={setRange} />
          </FormField>
        )}
        ...
        {approvalRequired ? (
          <FormField label="承認者" htmlFor="compensatory-leave-approver" required>
            <UserPicker id="compensatory-leave-approver" value={approverUserId} onChange={setApproverUserId} />
          </FormField>
        ) : (
          <p className="text-xs text-muted-foreground sm:col-span-2">
            現在の設定では代休申請に承認は不要です。申請すると同時に確定します。
          </p>
        )}
...
function MyCompensatoryLeaveRequestList() {
  ...
  if (requests.length === 0) {
    return <EmptyState title="代休申請はまだありません。" description="代休を申請すると、ここに一覧が表示されます。" />
  }
          ...
          <li key={req.id} className="flex items-center justify-between gap-3 py-3">
            ...
            {req.status === 'submitted' && (
              ...
              <ConfirmActionDialog
                ...
                title={`${req.target_date}の代休申請を取り消しますか?`}
 ...
 * 有給休暇(MyPaidLeavePage)・特別休暇(MySpecialLeavePage)と同じく、日付を指定するか
 * 期間を指定するかを選んで複数日をまとめて申請できる(ただし代休は日ごとに1件ずつ
 * 個別の消化申請として送信され、承認も日ごとに個別に行う)。
 */
  ...
  return (
    <div className="flex flex-col gap-6">
      <Card title="自分の代休">
        ...
        {grants.length === 0 ? (
          <EmptyState title="代休の付与はまだありません。" description="休日出勤の勤怠実績が確定すると、ここに一覧が表示されます。" />
        ) : (
          <Table>
            <TableHeader>
              <TableRow>

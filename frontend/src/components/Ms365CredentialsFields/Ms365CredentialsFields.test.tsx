...
import {
  ...
  type Ms365CredentialsFieldsValue,
} from "./Ms365CredentialsFields";
...
describe("Ms365CredentialsFields", () => {
  it("renders all fields with the given id prefix", () => {
    ...
    expect(
      screen.getByLabelText("クライアントシークレット"),
    ).toBeInTheDocument();
    expect(screen.queryByLabelText("リダイレクトURI")).not.toBeInTheDocument();
    expect(
      screen.getByLabelText("ローカル開発用モックOIDC(mock-oidc)を使う"),
    ).toBeInTheDocument();
  });

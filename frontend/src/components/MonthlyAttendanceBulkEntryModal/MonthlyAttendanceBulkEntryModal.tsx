...
/** 日付("YYYY-MM-DD")のISO曜日(月=1〜日=7)を返す。 */
function isoWeekdayOf(date: string): number {
  return ((new Date(`${date}T00:00:00`).getDay() + 6) % 7) + 1
}

/** 「まとめて設定」の内容を、対象月の日にちごとの行(「日にちごとに設定」タブの状態)に展開する。 */
function expandSimplePatternToDayOverrides(
  state: SimplePatternState,
  dates: string[],
): Record<string, DayOverrideRowState> {
  const rows: Record<string, DayOverrideRowState> = {}
  for (const date of dates) {
    rows[date] = {
      enabled: state.weekdays[isoWeekdayOf(date)] ?? false,
 ...
 * 「まとめて設定」タブではそのまま確定させず、必ず「日にちごとに設定」タブへ内容を
 * 展開してから確定させる(誤って月全体を一括で確定してしまう事故を防ぐ)。
 */
export function MonthlyAttendanceBulkEntryModal({
  ...
  const handleExpandToDetailed = () => {
    setDayOverrideState(expandSimplePatternToDayOverrides(simplePatternState, dates))
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent size="large" className="sm:max-w-3xl">
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'simple' | 'detailed')}>
          ...
          <TabsContent value="simple">
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              内容を確認のうえ確定するため、「次へ(日にちごとの内容を確認)」を押すと「日にちごとに設定」タブに展開されます。ここではまだ確定されません。
            </p>
          </TabsContent>
          <TabsContent value="detailed">
            <div className="grid grid-cols-1 gap-2">
              {dates.map((date) => {
                ...
                const weekdayLabel = WEEKDAYS[isoWeekdayOf(date) - 1].label
                return (
                  <div
                    key={date}
        ...
        {activeTab === 'simple' ? (
          <Button disabled={!from || !to} onClick={handleExpandToDetailed}>
            次へ(日にちごとの内容を確認)
          </Button>
        ) : (
          <Button

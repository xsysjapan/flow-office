...
describe("ConfirmActionDialog", () => {
  ...
  it("shows the title and description once opened", async () => {
    ...
    expect(
      await screen.findByText("端末を失効させますか?"),
    ).toBeInTheDocument();
  ...
  it("closes after an asynchronous confirmation succeeds", async () => {
    const user = userEvent.setup();
    render(
      <ConfirmActionDialog
        triggerLabel="解除"
        title="割当を解除"
        description="割当を解除します。"
        confirmLabel="解除する"
    ...
    expect(
      screen.queryByRole("dialog", { name: "割当を解除" }),
    ).not.toBeInTheDocument();
  });

  it("stays open after an asynchronous confirmation fails", async () => {
    const user = userEvent.setup();
    render(
      <ConfirmActionDialog
        triggerLabel="解除"
        ...
        description="割当を解除します。"
        confirmLabel="解除する"
        onConfirm={() => Promise.reject(new Error("解除に失敗"))}
      />,
    );
    ...
    expect(
      screen.getByRole("dialog", { name: "割当を解除" }),
    ).toBeInTheDocument();
  });

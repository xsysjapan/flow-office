...
1. **命名を決める**: `<aggregate>.<past_tense_verb>` 形式 (例: `attendance_punch.recorded`,
...
6. **通知が必要なら** `add-notification` スキルを使って通知ジョブをキューイングする。
...
承認イベントは状態遷移のトリガーに過ぎず、取り消す・書き換える操作は設けない。状態を

...
class SystemSettingController extends Controller
{
    ...
    #[OA\Put(
        ...
        summary: 'システム設定を更新する',
        tags: ['システム設定'],
        requestBody: new OA\RequestBody(required: true, content: new OA\JsonContent(required: ['default_timezone'], properties: [new OA\Property(property: 'default_timezone', type: 'string'), new OA\Property(property: 'default_work_style_id', type: 'integer', nullable: true), new OA\Property(property: 'attendance_submission_deadline_day', type: 'integer'), new OA\Property(property: 'attendance_month_close_deadline_day', type: 'integer'), new OA\Property(property: 'notification_mail_enabled', type: 'boolean'), new OA\Property(property: 'notification_mail_tenant_id', type: 'string', nullable: true), new OA\Property(property: 'notification_mail_client_id', type: 'string', nullable: true), new OA\Property(property: 'notification_mail_client_secret', type: 'string', nullable: true, description: '省略時は既存の値を変更しない'), new OA\Property(property: 'notification_mail_sender_address', type: 'string', nullable: true), new OA\Property(property: 'notification_mail_sender_name', type: 'string', nullable: true)])),
        responses: [new OA\Response(response: 200, description: 'Successful response'), new OA\Response(response: 401, description: 'Unauthenticated')],
    )]
    public function update(Request $request): SystemSettingResource
    {
        $data = $request->validate([
            ...
            'notification_mail_tenant_id' => ['nullable', 'string'],
            'notification_mail_client_id' => ['nullable', 'string'],
            'notification_mail_client_secret' => ['nullable', 'string'],
        ...
        // 既存の値を保持する。明示的に空文字を送っても消去はしない(無効化は notification_mail_enabled で行う)。
        if (! $request->filled('notification_mail_client_secret')) {
            unset($data['notification_mail_client_secret']);
        }

...
{
    #[OA\Get(
        path: '/employee-calendar-entries',
        operationId: 'employeeCalendarEntries.index',
     ...
     */
    #[OA\Post(
        path: '/employee-calendar-entries/generate',
        operationId: 'employeeCalendarEntries.generate',
        summary: '勤務予定を一括生成する',
        tags: ['勤務予定'],
    ...
    public function generate(Request $request, CommandBus $commandBus): AnonymousResourceCollection
    {
        ...
        $assignments = $commandBus->dispatch(new GenerateEmployeeCalendarEntries(
            userId: $data['user_id'],
     ...
     */
    #[OA\Post(
        path: '/employee-calendar-entries/preview-pattern',
        operationId: 'employeeCalendarEntries.previewPattern',
     ...
     */
    #[OA\Post(
        path: '/employee-calendar-entries/generate-pattern',
        operationId: 'employeeCalendarEntries.generatePattern',
    ...
    public function generatePattern(Request $request, CommandBus $commandBus): JsonResponse
    {
        $data = $request->validate([
            ...
            'overwrite_mode' => ['nullable', Rule::in([
                GeneratePatternCalendarEntries::OVERWRITE_MODE_SKIP_EDITED,
                GeneratePatternCalendarEntries::OVERWRITE_MODE_OVERWRITE_ALL,
            ])],
        ]);

        $result = $commandBus->dispatch(new GeneratePatternCalendarEntries(
            userId: $data['user_id'],
            workStyleId: $data['work_style_id'],
            from: $data['from'],
            to: $data['to'],
            ...
            overwriteMode: $data['overwrite_mode'] ?? GeneratePatternCalendarEntries::OVERWRITE_MODE_SKIP_EDITED,
            generatedByUserId: $request->user()->id,
        ));
     ...
     */
    #[OA\Post(
        path: '/employee-calendar-entries/assign-pattern',
        operationId: 'employeeCalendarEntries.assignPattern',
    ...
    public function assignPattern(Request $request, CommandBus $commandBus): JsonResponse
    {
        ...
        return (new EmployeeCalendarEntryResource($assignment))->response()->setStatusCode(201);
    }
     ...
     */
    #[OA\Get(
        path: '/employee-calendar-entries/review',
        operationId: 'employeeCalendarEntries.review',
     ...
     */
    #[OA\Post(
        path: '/employee-calendar-entries/publish',
        operationId: 'employeeCalendarEntries.publish',
        summary: '勤務予定を公開する',
        tags: ['勤務予定'],
    ...
    public function publish(Request $request, CommandBus $commandBus): JsonResponse
    {
        ...
        $result = $commandBus->dispatch(new PublishEmployeeCalendarEntries(
            userIds: $userIds,
     ...
     */
    #[OA\Put(
        path: '/employee-calendar-entries/{employeeCalendarEntry}',
        operationId: 'employeeCalendarEntries.update',
        summary: '勤務予定を編集する',
        tags: ['勤務予定'],
        parameters: [new OA\Parameter(name: 'employeeCalendarEntry', in: 'path', required: true, schema: new OA\Schema(type: 'string', format: 'uuid'))],
    ...
    public function update(Request $request, EmployeeCalendarEntry $employeeCalendarEntry, CommandBus $commandBus): EmployeeCalendarEntryResource
    {
        $data = $request->validate([
        ...
        $commandBus->dispatch(new EditEmployeeCalendarEntry(
            employeeCalendarEntryId: $employeeCalendarEntry->id,
        ...
        return new EmployeeCalendarEntryResource($employeeCalendarEntry->refresh());
    }

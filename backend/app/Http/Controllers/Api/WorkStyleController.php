...
class WorkStyleController extends Controller
{
    ...
    /**
     * 指示書 16.1節: 既存の勤務形態(初回オンボーディングで作成された標準の勤務形態を
     * 含む)の設定内容を変更する。コード・デフォルト指定・システム生成フラグは
     * このエンドポイントでは変更しない(UpdateWorkStyleHandler参照)。
     */
    #[OA\Put(
        path: '/work-styles/{workStyle}',
        operationId: 'workStyles.update',
        summary: '勤務形態を更新する',
        tags: ['勤務形態'],
    ...
    public function update(Request $request, WorkStyle $workStyle, CommandBus $commandBus): WorkStyleResource
    {
        ...
        $updated = $commandBus->dispatch(new UpdateWorkStyle(
     ...
     */
    private function validationRules(Request $request, ?string $ignoreId = null): array
    {
        $codeRule = Rule::unique('work_styles', 'code');
        if ($ignoreId !== null) {
        ...
        return [
            ...
            'company_calendar_id' => ['nullable', 'exists:company_calendars,id'],

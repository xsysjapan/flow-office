 ...
 *
 * @implements CommandHandler<EditEmployeeCalendarEntry>
 */
class EditEmployeeCalendarEntryHandler implements CommandHandler
{
    ...
    {
        assert($command instanceof EditEmployeeCalendarEntry);

        $assignment = EmployeeCalendarEntry::query()->with('workStyle')->findOrFail($command->employeeCalendarEntryId);
    ...
    private function assertWithinVariablePeriodCap(
        EmployeeCalendarEntry $assignment,
        ...
        $otherAssignmentsTotalMinutes = EmployeeCalendarEntry::query()
            ->where('user_id', $assignment->user_id)
            ...
            ->get()
            ->sum(fn (EmployeeCalendarEntry $other) => $other->plannedWorkMinutes());

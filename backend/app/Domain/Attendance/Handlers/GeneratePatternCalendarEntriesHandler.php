 ...
 * 展開する。既存の`EmployeeCalendarEntryAssigned`イベント・集約・Projectorを再利用し、
 * `GenerateRotationCalendarEntriesHandler`と同じ安全策(実績のある日・締め済みの日は
 ...
 *
 * @implements CommandHandler<GeneratePatternCalendarEntries>
 */
class GeneratePatternCalendarEntriesHandler implements CommandHandler
{
    ...
    {
        assert($command instanceof GeneratePatternCalendarEntries);
        ...
        foreach ($period as $date) {
            ...
            if ($existing?->is_manually_overridden && $command->overwriteMode === GeneratePatternCalendarEntries::OVERWRITE_MODE_SKIP_EDITED) {
                $skipped[] = $date->toDateString();

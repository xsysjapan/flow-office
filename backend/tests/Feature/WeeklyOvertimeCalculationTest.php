 ...
 * 避けつつ、attendance_weekly_calculations Projectionへ正しく反映されることを確認する。
...
class WeeklyOvertimeCalculationTest extends TestCase
{
    ...
    private function weeklyCalculation(User $user, string $weekStartDate): AttendanceWeeklyCalculation
    {
        return AttendanceWeeklyCalculation::query()
    ...
    public function test_weekly_overtime_is_captured_even_when_no_single_day_exceeds_eight_hours(): void
    {
        ...
        $weekly = $this->weeklyCalculation($user, '2026-06-01');

        $this->assertSame(2520, $weekly->actual_work_minutes);
        $this->assertSame(0, $weekly->daily_statutory_overtime_minutes);
        $this->assertSame(120, $weekly->weekly_statutory_overtime_minutes);
    }
    ...
    public function test_daily_and_weekly_statutory_overtime_are_not_double_counted(): void
    {
        ...
        $this->assertSame(120, $weekly->daily_statutory_overtime_minutes, '金曜の日8時間超(2時間)のみ');
        $this->assertSame(0, $weekly->weekly_statutory_overtime_minutes, '日次で計上済みの時間を除けば週40時間ちょうどのため0');
    }
    ...
    public function test_company_holiday_work_is_included_in_the_weekly_forty_hour_threshold(): void
    {
        ...
        $weekly = $this->weeklyCalculation($user, '2026-06-01');
        $this->assertSame(60, $weekly->daily_statutory_overtime_minutes);
        $this->assertSame(480, $weekly->weekly_statutory_overtime_minutes, '週40時間超過分(合計49時間-40時間-日次計上済み1時間)');
    }
    ...
    public function test_legal_holiday_work_is_excluded_from_the_weekly_forty_hour_aggregation(): void
    {
        ...
        $this->assertSame(2400, $weekly->actual_work_minutes, '法定休日労働は週の実働集計に含めない');
        $this->assertSame(0, $weekly->weekly_statutory_overtime_minutes);
        $this->assertSame(300, $weekly->legal_holiday_work_minutes);
    }

    public function test_monthly_snapshot_includes_the_weekly_statutory_overtime_total(): void
    {
        $calendar = $this->makeCalendar();
        $workStyle = $this->makeWorkStyle($calendar);
        $user = User::factory()->create();
        ...
        $response->assertSuccessful()->assertJsonPath('snapshot.weekly_statutory_overtime_minutes', 120);
    }
}

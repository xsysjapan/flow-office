...
7. 以降、端末アプリは`POST /devices/heartbeat`を10分に1回程度の間隔で送り、定期的に疎通確認
   (`last_seen_at`/`app_version`の更新)を行う。これは高頻度な運用テレメトリでありCommand/
   EventStoreを経由しない(業務上の事実ではないための意図的な例外。system_settings/
   request_typesと同じ「マスタ的な設定の直接更新」の扱いに準ずる)
8. 管理者が端末管理画面で最終通信日時が更新されたことを確認する。`status=active`のまま
   最終通信から30分以上経過している場合、画面側で「疎通途絶」と表示し、実際には稼働して
   いない可能性を管理者に知らせる(`status`自体はサーバー側で書き換えない。あくまで
   画面表示上の判定であり、日次の警告通知は別途`WarnStaleDevicesHandler`(cron)が担う)

### 再ペアリング(ペアリング済み端末の復旧)

Androidアプリの削除・端末の初期化などで本トークンを失った場合、`status=active`のまま
サーバー側にはそれを検知する手段がない。この状態から打刻できるようにするため、管理者は
ペアリング済み(`active`)の端末に対しても`POST /devices/{device}/pairing`を呼び直せる。
...
社員証NFCのIDは事前には分からず、端末でスキャンするまで判明しない。そのため
Web管理画面からIDを直接入力させるのではなく、共有Android端末(`device_roles`に
`authentication_device`等を持ち、`device_scopes`に`admin:mode`を付与された端末)上で
管理者本人を確認したうえで、その場でスキャンしたNFCを対象社員に登録できるようにする。

### 初回登録(ブートストラップ)

端末をアクティベーション(UC-D002のペアリング)した直後は、管理者ICカードが
1枚も登録されていない状態がありうる。この場合でも、以下の経路で「一時的に」
(=ゼロ件かどうかで機能を出し分けるのではなく、アクティベーションを行った管理者権限を
根拠に常に)登録を開放する。
...
3. 登録と同時にその端末を管理者モードにする(`device_admin_sessions`、
   `device_admin_session.started`、source=`bootstrap`)
...
3. 管理者モード中は、社員一覧(`GET /devices/me/admin/users`)・対象社員の既存の
   ...
   対象社員を選んでスキャンしたNFCを登録できる
   (`POST /devices/me/admin/users/{user}/authentication-keys`)
4. 管理者モードは有効期限で自動的に終了するほか、`POST
   /devices/me/admin-sessions/current/end`で明示的に終了できる。新たにカードをかざすと
   既存のセッションは終了し、新しいセッションに置き換わる

関連イベント: `device_admin_session.started`, `device_admin_session.ended`,
`authentication_key.issued`
関連テーブル: `devices`(`activated_by_user_id`), `device_admin_sessions`, `authentication_keys`
...
- 端末一覧(端末名・所有区分・端末種別・役割・事業所・状態・最終通信・アプリバージョン)。
  一覧行から直接行える操作は「詳細へ遷移」「失効」「削除」のみとし、行クリックでも詳細を
  開けるようにする。QRコード表示・停止・再ペアリングなどそれ以外の操作は端末詳細に集約する
- 端末詳細、共有端末追加、ペアリング用QR(claim token)表示、再ペアリング(UC-D002)
- 端末権限(役割・スコープ)設定、設置場所設定。共有端末登録時に選べる役割

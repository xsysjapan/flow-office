...
既存のMicrosoft SSOトークン交換フロー(`docs/06-usecases-auth.md` UC-001、
`POST /auth/token`)と同じ「一度きりのコードをSanctumトークンに交換する」パターンを、
端末ペアリングにも流用する。非対称鍵(端末が鍵ペアを生成しサーバーへ公開鍵を登録する方式)は
採用せず、既存のSanctum Bearerトークン基盤に寄せることで新しい署名検証基盤を増やさない。

1. 管理者がUC-D001で登録した端末に対し、ペアリングコード(またはQRコード)を発行する
   (`POST /devices/{device}/pairing`。有効期限付き、一度使用したコードは再利用できない)
2. Androidアプリ(または対象端末)がコードを読み取り、`POST /devices/pairing/exchange`へ
   コードを送信する
3. サーバーはコードを検証し、対象`devices`行の`device_roles`に応じたability
   (例: `ATTENDANCE_READER`役割なら`recorder:punch`)を持つSanctumトークンを発行する
   (`$device->createToken('device', $abilities)`。`devices`モデルにもSanctumの
   `HasApiTokens`を付与し、User以外の任意のEloquentモデルにもトークンを発行できる
   Sanctumの仕組みをそのまま使う)
4. 平文トークンは端末アプリへの応答として一度だけ返す(以降、端末はこのトークンを
   `Authorization: Bearer`として全リクエストに付与する)。この時点で`status=active`にする
5. `device.paired`イベントを記録する
6. 以降、端末アプリは`POST /devices/heartbeat`で定期的に疎通確認(`last_seen_at`/
...
7. 管理者が端末管理画面で最終通信日時が更新されたことを確認する
...
5. サーバーが、本人の打刻専用ability(`punch:self`)のみを持つSanctumトークンを発行する
   (UC-D002のペアリングコード交換は不要。既にログイン済みの本人操作のため)

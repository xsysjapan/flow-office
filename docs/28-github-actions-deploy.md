...
起動する。`deploy` ジョブは `test-backend`(`.github/workflows/tests.yml` の再利用)・
`test-frontend`(`.github/workflows/frontend.yml` の再利用)の両方が成功するまで実行されない
(`needs:`)。テストが失敗した場合はビルド・転送・切替のいずれも行われない。

0. backend/frontendのテスト・lintがすべて成功していること(`test-backend`・`test-frontend`)。
...
XSERVER側のSSHセッションは(回線・共有ホスティング側の一時的な制限により)
`Connection closed by ...`で突然切断されることがある。`deploy.yml`の各SSH/rsync/scp呼び出しは
`deploy/scripts/ci-retry.sh`の`retry`関数(指数バックオフで最大5回まで再試行)でラップして
一時的な切断を吸収している。ただし`activate-release.sh`の実行(migrate・cache・`current`切替)
自体は多重実行を避けるため自動リトライの対象外にしている。この呼び出しが切断で失敗した場合は
`workflow_dispatch`で手動再実行すること(スクリプト自体は再実行しても安全な作りになっている)。

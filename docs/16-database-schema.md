...
のみを付与する(docs/23-usecases-devices.md「外部端末登録」参照)。
...
## 月次勤怠インポート・下書き(docs/26-usecases-monthly-import.md)

作業報告書等から月次勤怠を作成する機能専用のテーブル群。既存の`attendance_months`
(正式な月次勤怠、承認・締めの対象)とは分離し、下書き段階では`attendance_months`を
一切更新しない(docs/03-architecture.md 3.6「打刻と勤怠編集を区別する」、
CLAUDE.mdの設計原則10)。

### monthly_attendance_drafts (月次勤怠下書き。正データ)

- id
- user_id
- target_month (`YYYY-MM`)
- status (`draft` / `validating` / `needs_review` / `ready_to_submit` / `submitted` /
  `approved` / `rejected` / `locked`)
- version (楽観ロック用のバージョン番号。Web画面・Claude(MCP経由)からの同時編集で
  後勝ち上書きを防ぐ。更新APIは`expected_version`を受け取り、一致しない場合はHTTP 409
  相当のエラー(`ATTENDANCE_VERSION_CONFLICT`)を返す)
- source_type (`source_document` / `existing_clock_event` / `manual` 等。下書き全体の
  主な作成元)
- source_reference (nullable。`attendance_import_sessions.id`等、作成元の参照)
- created_by_user_id
- submitted_at (nullable)
- created_at / updated_at

下書きは`monthly_attendance_drafts`単体では確定した勤怠にならない。ユーザーの明示的な指示
(「申請して」)によってのみ、既存の月次勤怠提出フロー(UC-A008)へ引き渡される
(docs/26「月次申請」参照)。

### attendance_import_sessions (作業報告書インポートセッション。正データ)

- id
- user_id
- target_month
- status (`created` / `previewing` / `previewed` / `applied` / `cancelled`)
- source_type (`work_report` 等)
- source_file_name (nullable)
- source_file_hash (nullable。同一資料の重複インポート検知に使う)
- client_type (`ai_application` 等。`application_integrations`経由の場合は下記integration_id)
- integration_id (nullable。`application_integrations`への外部キり)
- created_at / updated_at

ファイル解析自体(Excel/CSV/PDF/Word/画像等の汎用パーサ)は勤怠管理API・MCPサーバーの
責務にしない(Claude側で解析済みの構造化データを受け取る。docs/26「作業報告書から月次勤怠を
作成するフロー」参照)。

### attendance_import_items (インポート日別明細)

- id
- import_session_id
- work_date
- proposed_data_json (報告書から抽出した日別勤務候補: 開始・終了・休憩・勤務場所・案件・
  作業内容・信頼度・元資料参照等)
- existing_data_json (対象日の既存勤怠(`attendance_days`/`attendance_punches`/勤務予定等)の
  スナップショット)
- differences_json (既存データとの差異一覧。docs/26「差異検出」の検出項目)
- validation_result_json (`AttendanceCalculator`等、既存の勤怠ルール検証結果)
- confidence (`high` / `medium` / `low`)
- status (`pending_review` / `confirmed` / `excluded`)
- source_reference_json (元資料のシート名・行番号等)
- created_at / updated_at

### field_provenances (項目ごとの値の出所)

`monthly_attendance_drafts`・`attendance_import_items`等の各入力項目について、値の生成元を
保持する(docs/03-architecture.md 3.7「AIは勤怠ルールを決定しない」)。

- id
- entity_type (`monthly_attendance_draft` / `attendance_import_item` 等)
- entity_id
- field_name (例: `start_time`, `end_time`, `work_location_type`)
...
- source_reference_json (nullable。元資料の参照、AI推定理由等)
- confidence (nullable)
- previous_value (nullable。変更前の値)
- confirmed_by_user_id (nullable)
- confirmed_at (nullable)
- created_at

`source_type=ai_inferred`のまま`confirmed_at`が未設定の重要項目(出勤時刻・退勤時刻・休憩
時間・勤務日・休日勤務・休暇との競合)が対象月に1件でも残っている場合、月次申請
(`SubmitMonthlyAttendanceDraft`)を拒否する(`AI_INFERRED_VALUE_UNCONFIRMED`エラー、
docs/26参照)。

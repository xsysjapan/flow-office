...
- late_night_minutes
- regular_work_late_night_minutes (深夜のうち所定内労働にあたる分。late_night_minutesの内訳)
- non_statutory_overtime_late_night_minutes (深夜のうち所定内残業にあたる分。late_night_minutesの
  内訳)
- statutory_overtime_late_night_minutes (法定外深夜。statutory_overtime_minutesのうち深夜時間帯
  ...
  statutory_overtime_late_night_minutes = late_night_minutes)
...
(`prescribed_work_minutes`/`non_statutory_overtime_minutes`/`statutory_overtime_minutes`/
`late_night_minutes`/`legal_holiday_work_minutes`/`company_holiday_work_minutes`)も、
...
対象月全体の9区分(実働・所定内残業・法定外残業・60時間以内/超・深夜(所定内労働/所定内残業/
法定外残業の内訳含む)・法定休日労働・所定休日労働・法定休日深夜)を合算した結果を

...
所有しない。ユーザー作成後の`ALL_USERS`所属のように複数境界をまたぐ処理は、
...
`/api/admin/access-control/*`に分ける。管理画面では、人事担当者が扱うユーザー・グループ・所属・
所属変更・外部HR取込を「人事・組織」に配置し、誰が何を利用できるかを決めるFeature・Role・Permission・
個別停止、外部ID・項目管理元、GroupTypeを「システム」に配置する。画面とルートは責務ごとに分離するが、
共通部品は再利用し、フロントエンドのAPIクライアントと状態管理も`userManagement`と`accessControl`に分離する。
...
従来の`users`と`roles`の多対多関連（`role_user`）および`User.roles` APIは廃止する。
既存の`role_user`は移行時に同等のユーザー直接`RoleAssignment`へ変換してからテーブルを削除する。
以後、Roleの付与元は`role_assignments`だけとし、画面表示、メニュー表示、API認可、ドメイン内判定の
いずれも旧ユーザーロールや固定ロールコードを認可根拠にしない。`user.roles_changed`と
`user.roles_migrated_from_legacy`のイベント型は過去のStoredEventを復元するためだけに残し、新規には発行しない。
...
- ユーザー一覧・詳細: 「人事・組織」に配置する。氏名、社員番号、メール、主所属、雇用区分、在籍状態、
  現在所属、予約変更、働き方等の人事情報を表示・編集する。グループ種別、在籍状態、アカウント状態は
  内部コードではなく日本語名称で表示する。ユーザー詳細では現在所属と予約済み変更を「グループ所属」にまとめ、
  所属の追加・解除・同一グループ種別内の移動・主所属変更ごとに、即時反映または日時指定予約を選択できる。
  同じ領域の「変更履歴」には、即時変更と予約変更を更新日時の降順で最大20件表示する。各履歴には
  操作種別と対象グループ名（移動の場合は変更元・変更先）を主情報として日時より一段目立たせ、適用日時は補助情報として
  小さく表示する。状態名は日本語で表示し、下書き・予約済みの変更は履歴から取り消せるようにする。
  主所属は同じグループ種別の中で代表として扱う所属であることを画面内で説明する。
  有効Feature、Role/Permission、付与元、個別停止、
  外部ID、項目管理元、認証キーは表示・編集しない。
- グループ管理: 「人事・組織」に配置する。グループ一覧は検索・絞り込み・一括処理と詳細画面への導線に限定し、
  名称、親グループ、状態等の個別編集と所属メンバーの追加・解除はグループ詳細画面で行う。メンバーの
  追加・解除は即時反映または日時指定予約を選択できる。グループ詳細の変更履歴には、そのグループを対象とする
  即時変更・予約変更を最大20件表示し、操作種別、対象グループ名、ユーザー名、日本語の状態名を表示する。
  下書き・予約済みの変更は履歴から取り消せるようにする。
  グループコードはサーバー側で内部生成し、一覧・詳細・作成画面には表示しない。一覧のメンバー列は人数だけを表示する。
  グループへ付与されたFeature・Role等のアクセス情報は
  人事担当者向け画面へ表示しない。
- 所属変更: 「人事・組織」の独立画面に、複数明細を扱う所属変更の下書き・予約・適用状況一覧を配置する。
  一覧にはユーザー名、操作種別、対象グループ名、適用日時、日本語の状態名を表示し、下書き・予約済みの
  変更は取り消せるようにする。予約登録フォームは常時表示せず、「変更予約作成」ボタンからモーダルで表示する。
  下書き・予約済みの変更も一覧の「変更」から同じ形式のモーダルを開き、内容を変更できるようにする。
  ユーザー詳細・グループ詳細からも、それぞれに関係する履歴を確認できる。
- 人事データ連携: 「人事・組織」の独立画面に、外部HR CSV差分取込だけを配置する。
  狭い画面では入力欄を1列にし、差分表だけを内部横スクロールさせる。
- アクセス管理: 「システム」の独立画面にFeature割当・解除、Role・Permission、RoleAssignment、
  ユーザー直接Role、個別Feature停止を配置し、「Feature」「Role・Permission」「個別停止」のタブに分ける。
  変更頻度の低い編集操作にはモーダルまたは確認ダイアログを使用する。
- ID・管理元設定: 「システム」の独立画面に外部ID、項目管理元、認証キーを配置する。
- グループ種別管理: 変更頻度が低いため「システム」の独立画面に配置する。人事担当者はグループ編集に必要な
  グループ種別一覧を参照できるが、グループ種別の追加・編集はできない。
- 権限境界: 人事担当者にはFeature・Role・Permission・外部ID・項目管理元・認証キーの管理権限を付与しない。
  メニュー非表示と直URLリダイレクトに加え、各APIでもPermissionを検証して403を返す。
  人事担当者向けFeatureはシステムグループ`HUMAN_RESOURCES_USERS`へ付与し、システム管理Featureとは分離する。
- 操作安全性: 所属・Feature・Role割当・個別停止の解除、グループ無効化等は確認ダイアログを挟む。
  編集画面やモーダル内で失敗した更新のエラーはその場に表示し、入力内容を保持して再試行できるようにする。
...
- 人事担当者には「ユーザー」「グループ」「所属変更」「人事データ連携」だけを表示し、「アクセス管理」
  「ID・管理元設定」を表示しない。両画面への直URLアクセスと、Feature・Role・Permission・外部ID・
  項目管理元の管理API直接呼出しが拒否されること。

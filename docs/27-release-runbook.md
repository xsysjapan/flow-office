...
| `/flow-office/mcp` | `mcp/` | MCPサーバー。JSON-RPCエンドポイントは`/flow-office/mcp/mcp` |
...
curl -I -X POST https://example.com/flow-office/mcp/mcp                   # 401 (トークン無し。404/405/500ではないこと)
```
...
- **`route:cache`とURLサブパス配置の組み合わせで、ルート`'/'`のGETが誤って405になる**:
  ...
  使う前提でサブパス配置するアプリでは、**文字通りの`'/'`にルートを登録しない**設計にする
  こと(案内が必要な場合はWebサーバー側のリダイレクトで対応する)。

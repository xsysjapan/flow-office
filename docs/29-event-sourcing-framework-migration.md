...
4. PaidLeave / SpecialLeave(複数集約書き込みが絡む。`persistInTransaction`の実地検証を兼ねる)
   — **完了**(詳細は下記「移行済み: PaidLeave / SpecialLeave」)
5. User — **完了**(詳細は下記「移行済み: User」)。`users.id`はUUID化する判断とした
   (28件のマイグレーション・Sanctum認証・フロントエンドのユーザーID表示に影響するが、
   他ドメインと主キー方式を統一する)。
6. Attendance(最大・最も複雑なため最後。サブ集約が多数あるため複数回に分けて移行した)
   — **完了**(詳細は下記「移行済み: Attendance」)
   - **【解消済み】`AttendanceDailyCalculationProjector`のspatie形式への変換**:
     旧イベントバス(`App\Domain\EventSourcing\StoredEventRecorded` →
     `app/Listeners/ProjectStoredEvent.php` → `config('domain.projectors')`)への
     最後の残存購読者だったが、Attendance移行と同時に
     `Spatie\EventSourcing\EventHandlers\Projectors\Projector`のサブクラスへ書き換え、
     `config('domain.projectors')`からも除去した。同配列は現在空になっている
     (旧イベントバスの仕組み自体・`ProjectStoredEvent`リスナーは、他ドメインへの影響有無を
     再確認できていないため今回は削除していない)。
...
## 移行済み: PaidLeave / SpecialLeave

`persistInTransaction`の実地検証を兼ねた段階(DeviceAdminSessionに次ぐ2・3例目)。
両ドメインは「申請(request)集約」と「付与(grant)集約」の2つの集約ルートを持ち、
承認1件で両方にまたがる書き込みが発生する点が今までの移行対象と異なる。

- `paid_leave_grants.id` / `paid_leave_requests.id` / `special_leave_grants.id` /
  `special_leave_requests.id` をUUID主キー(`HasUuids`)に変更した(承認・差戻し・取消の
  各コマンドがこのIDを使って集約を再取得するため、`workflow_requests`等と同じ理由で
  UUID化が必要。`paid_leave_usages` / `special_leave_usages`側のFK列も`foreignUuid`に変更)。
- **`paid_leave_usages` / `special_leave_usages`は集約ルートではない**: これらの行に対して
  発行される後続コマンドは存在しない(1回作られたら不変)ため、主キーは従来通りDB採番のまま
  でよい。代わりに`stored_event_id`列(nullable、unique)を追加し、`WorkflowRequestHistoryEntry`
  と同じ考え方でProjectorの冪等性(同一イベント再適用時の重複防止)を担保する
  (nullableにしているのは、テストがイベントを経由せず直接rowを作成することがあるため)。
- イベントから`paidLeaveGrantId`/`paidLeaveRequestId`/`paidLeaveUsageId`等の
  自己参照的なID引数を削除し、`$event->aggregateRootUuid()`を使う形にした
  (Workflow/BackOfficeと同じ整理)。あわせて、旧実装がHandler側で直接rowに書き込んで
  いたためイベントに含まれていなかったフィールド(`PaidLeaveGranted.grantReason`、
  `PaidLeaveRequested.hours`/`reason`、`PaidLeaveUsed.attendanceDayId`/`usedMinutes`/
  `usageType`、SpecialLeave側も同様)を追加し、Projectorがイベントだけから行を完全に
  再構築できるようにした。
- **`PaidLeaveGrantProjector`/`SpecialLeaveGrantProjector`の`used_days`/`remaining_days`は、
  その集約に記録された全`*.used`イベントの`usedDays`合計をstored_eventsから都度再計算する**
  (加算・減算の差分更新ではなく、常に全件から再計算する)。差分更新だと同じイベントの
  再適用(リプレイ)で二重計上されるが、集計方式なら再適用しても同じ結果になる
  (`.claude/skills/add-projection`のProjector冪等性チェックリストに対応)。
- **`ApprovePaidLeaveRequestHandler`/`ApproveSpecialLeaveRequestHandler`は
  `AggregateRoot::persistInTransaction()`で「申請の承認」+「1件以上のgrantの消化」を
  1トランザクションにまとめて記録する**。消化計画(どのgrantから何日消化するか)を
  先にすべて確定させ、残数不足ならどの集約にもイベントを記録せず例外を投げる形にした。
  旧実装はgrantごとに逐次「消化してsave→残数不足なら例外」としていたため、途中のgrantまで
  消化を反映してから例外を投げてしまう(トランザクションで守られていない)不整合が
  実は存在していた。この整理により副次的に修正された。
- **`LeaveHistoryQuery`(有給・特別休暇の履歴表示で共通の読み取り専用Query)を
  新テーブル(`stored_events`)のみを検索する形に単純化した**: 両ドメインの移行が完了した
  ため、`AuditLogController`の時と同じ理由で旧`legacy_stored_events`側の検索が不要になった。
  新テーブル側は`event_class`が集約ごとに分かれない(grant/requestどちらも`"paid_leave."`
  始まりの同じ接頭辞)ため、aggregate_typeでの絞り込みをやめ、対象社員が実際に持つ
  grant/requestのUUID一覧で`aggregate_uuid`を直接絞り込む形にした。`event_properties`は
  イベントクラスの公開プロパティ名(camelCase)のままなので、フロントエンドのレスポンス
  形状(`payload`のsnake_caseキー)を変えないよう、返却時に`Str::snake()`で変換している。
- フロントエンド`PaidLeaveGrant.id`/`PaidLeaveRequest.id`/`SpecialLeaveGrant.id`/
  `SpecialLeaveRequest.id`型を`number`→`string`に追随(`frontend/src/api/types.ts`、
  関連するAPI関数・hooks・テスト・story)。履歴表示(`fetchMyPaidLeaveHistory`等が返す
  `StoredEvent[]`の形)自体は変わっていないため、`LeaveHistoryList`コンポーネント側の変更は
  不要だった。

## 移行済み: User

`users.id`のUUID化を伴う、作業順のうち最も影響範囲が広い段階(28件のマイグレーションの
FK・Sanctum認証・フロントエンドのユーザーID表示すべてに波及する)。

- **`users.id`をDB採番からUUID主キー(`HasUuids`)に変更した**。他のES管理集約と
  主キー方式を統一する判断(据え置き案もあったが、集約IDのUUID化方針に合わせることを
  優先した)。これに伴い、`users.id`を外部キーとして参照する全マイグレーション
  (`foreignId` → `foreignUuid`、28ファイル)・全ドメインの`int`型user-idプロパティ
  (Command/Event/Aggregate/Handler/Controller・約140ファイル)・フロントエンドの
  `User.id`型およびuser-id系フィールド(`api/types.ts`・hooks・pages・components・
  テスト/story・99ファイル)を`string`に統一した。他エンティティ固有の連番PK
  (`work_style_id`・`attendance_day.id`・`request_type.id`等)は対象外。
  `personal_access_tokens.tokenable_id`は元々`uuidMorphs`だったため、User側の変更で
  スキーマ変更は不要だった(コメントのみ更新)。
- User集約は8種類のイベントを持つ: `UserOnboardedAsAdmin`(初期管理者オンボーディング)、
  `UserCreatedFromSsoLogin`(SSO初回ログインで既存ユーザーが見つからない場合の新規作成。
  旧実装には存在しなかった新設イベント)、`UserSyncedFromMs365`、`UserSsoAccountLinked`、
  `UserLoggedIn`、`UserRolesChanged`、`UserHireDateSet`、`UserTerminationDateSet`。
- **`UserOnboardedAsAdmin`はパスワードをイベントペイロードに含めない**: 他のイベントは
  Projectorがイベントのみから行を完全に再構築できることを原則としているが、パスワードは
  平文・ハッシュ問わず追記専用でリプレイ可能なイベントログに永続保存すべきでないため、
  意図的な例外としている。`CompleteOnboardingWithLocalPasswordHandler`は
  `UserAggregate::onboardAsAdmin()`で`entraUserId: null`のイベントを記録した後、
  `User`モデルへ`password`を直接設定・保存する(イベント経由ではない)。
- **`UserLoggedIn.loggedInAt`は明示的なフィールドとして持たせている**: `last_login_at`は
  `system_settings.default_timezone`を用いた`LocalDateTime::now($tz)`で記録する必要があり
...
- `CompleteOnboardingSsoLinkHandler`で`SystemSetting::completeOnboarding()`の判定を
  ...
  旧実装に存在していた。`ApprovePaidLeaveRequestHandler`で見つかったのと同種の
  「トランザクションで守られていない途中状態」の修正)。
- `SyncUsersFromMs365Handler`は`wasCreated`判定・手動`create()`/`fill()`/`save()`分岐を
  廃止し、`UserAggregate::syncFromMs365()`の単一呼び出しに統一した
  (`UserProjector::onUserSyncedFromMs365`の`updateOrCreate`が新規/更新どちらも処理する)。

## 移行済み: Attendance

作業順の最後・最大のドメイン。11個のテーブル・8集約・約34コマンド/26イベント/34ハンドラを
対象に、3回の作業(コミット)に分けて移行した。
...
- **UUID化した11テーブル**(いずれもフォローアップコマンドが自分自身のidを再度対象にする、
  ...
  `special_leave_grant_rules.work_style_id`・`system_settings.default_work_style_id`を含む)を
  `foreignUuid`へ変更した。
...
- **8つの集約**: `AttendanceDay` / `AttendancePunch` / `AttendanceMonth` / `WorkCalendar` /
  ...
  既存Handlerを`XxxAggregate::retrieve($id)->method(...)->persist()`経由に書き換えた。
- **`AttendanceDailyCalculationProjector`のspatie形式への変換**(作業順6の【課題】として
  事前に記載していた本題): 旧`App\Domain\EventSourcing\Contracts\Projector`実装から
  `Spatie\EventSourcing\EventHandlers\Projectors\Projector`のサブクラスへ書き換え、
  `onAttendanceDayCalculated`/`onAttendanceDailyCalculationAdjusted`で
  `attendance_daily_calculations`を更新する形にした。`config/domain.php`の`projectors`配列から
  このProjectorのエントリを除去し、同配列は現在空になっている。
- **打刻(punch)は追記専用原則を維持**(CLAUDE.md原則10): `AttendancePunchCorrected`/
  `AttendancePunchDeleted`は既存の打刻行を書き換えない。Projector側で元の行の`status`を
  `corrected`/`deleted`に更新し、訂正の場合はイベントに埋め込まれたデータから
  新しい行(`correctedPunchId`)を作成する形にした(新しい行自身は独立したイベントストリームを
  持たない)。
- **打刻⇔日次実績のクロス集約トランザクション**: `RecordAttendancePunchHandler`等は、
  打刻の変更を反映した「有効な打刻一覧」をメモリ上に組み立てたうえで
  `AttendanceDayPunchSyncer::prepare()`に渡し、まだ永続化していない`AttendanceDay`集約
  (DBに存在しない打刻を前提にした一時的なEloquentモデル。`AttendanceCalculator`は
  Eloquent属性/リレーションしか読まないため成立する)に対して`syncLiveStatus`/
  `syncFromPunches`/`calculate`/`autoInsertBreak`イベントを記録させる。打刻集約と日次実績集約を
  `AggregateRoot::persistInTransaction()`で同時に確定させることで、打刻がまだDBに無い状態でも
  両者を1トランザクションでアトミックに書き込めるようにした。
- **`DesignateLegalHolidayHandler`の直接反映+`persistInTransaction`**: `LegalHolidayResolver`が
  `legal_holiday_designations`をDBから直接読んで法定休日判定を行う都合上、週内の日次再計算を
  行う前に指定内容をこのテーブルへ直接反映しておく必要がある。PaidLeaveの
  `$day->work_type`直接反映と同じ理由による許容パターンとして、指定行への直接`save()`の後、
  `LegalHolidayDesignationAggregate`+週内の対象`AttendanceDayAggregate`群のイベントを
  `persistInTransaction()`でまとめて記録する(Projectorの`updateOrCreate`が同じ内容を
  再度書き込むため冪等)。
- **1ユーザー1行系アグリゲート**: `EmployeeRotationAssignment`(社員ごとに1行)・
  `UserWorkStyleMonthlyAssignment`(社員×年月ごとに1行)は、既存行があればそのidを
  集約ストリームIDとして再利用する形にした(再割り当てのたびに新規UUIDを発行しない)。
- **副次的に発見・修正した既存バグ**: `PaidLeaveController::storeRule`/
  `SpecialLeaveController::storeRule`/`SystemSettingController::update`の
  `work_style_id`/`default_work_style_id`バリデーションが`work_styles.id`のUUID化後も
  `integer`のまま残っており、有効なUUIDを422で弾いていた(テストがnull以外の値を
  渡すケースを持たなかったため気づかれていなかった)。`string`へ修正した。
- フロントエンドの11エンティティ分のid型(`number`→`string`)・関連するFK参照を
  `frontend/src/api/types.ts`ほか関連するAPI関数・hooks・pages・components・テスト・storyへ
  追随させた。

移行後、フルテストスイート364/364・`./vendor/bin/pint --dirty`・
`php artisan migrate:fresh --seed`をすべて確認済み。

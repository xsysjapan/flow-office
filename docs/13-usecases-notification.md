...
| 月次締め前警告 | `system_settings.attendance_month_close_deadline_day`の直前(既定3日前)になっても前月分が締められていない | 人事部(対象社員のバックオフィスタスク担当者。未割当なら人事部担当者全員) | 解消するまで毎日(既存踏襲) |
...
「勤怠未提出」「月次締め前警告」は、対象社員の`users.usage_start_date`(利用開始日)が
未設定、または`usage_start_date`・`users.hire_date`(入社日)が対象月より後の場合は
フォロー対象外とする(まだ本システムの利用や在籍を開始していない、またはそれが確認できて
いないため)。`usage_start_date`を未設定のまま「制限なし」として扱うと、入社前月分の
督促が誤って送られる事故につながるため、未設定は「対象外(送信しない)」として扱う
(`hire_date`は未設定でも制限しない。あくまで`usage_start_date`が基準)。有給・特別休暇の
自動付与(docs/09-usecases-paid-leave.md UC-P002)についても同様に、利用開始日が設定されて
いてそれが未到来の場合は付与・通知の対象外とする。

`usage_start_date`は、新規ユーザー作成時(SSO初回ログイン・オンボーディング・MS365同期での
新規作成)にそのユーザー行が作成された日をデフォルト値として自動設定する。管理者は実際の
入社日・利用開始日が判明次第、個別に上書き修正できる。ただし本番カットオーバー移行時の
合成イベント(`UserMigratedFromLegacy`)で作成されたユーザーや、この項目の導入(2026-07-25)
より前から在籍する社員は対象外で、`usage_start_date`が未設定のままの場合がある。その間は
上記の「対象外(送信しない)」という扱いになるため、必要な社員には管理者が`usage_start_date`
を個別に設定すること。

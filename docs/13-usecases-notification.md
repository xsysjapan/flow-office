...
「勤怠未提出」「月次締め前警告」は、対象社員の`users.usage_start_date`(利用開始日)・
`users.hire_date`(入社日)が対象月より後の場合はフォロー対象外とする(まだ本システムの
利用や在籍を開始していないため)。いずれも未設定の場合は制限しない。有給・特別休暇の
自動付与(docs/09-usecases-paid-leave.md UC-P002)についても同様に、利用開始日が設定されて
いてそれが未到来の場合は付与・通知の対象外とする。

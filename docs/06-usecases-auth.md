...
## UC-004: ローカルパスワードユーザーがMicrosoft 365アカウントと連携する

初回オンボーディング(UC-000)でローカルパスワードモードを選んだ場合や、SSOログイン(UC-001)
と紐づかないまま運用してきた場合でも、ログイン中のユーザーが任意のタイミングで自分の
アカウントにMicrosoft 365(Entra ID)アカウントを紐づけられる。

1. ローカルパスワードでログイン中のユーザーが「アカウント設定」画面で連携を開始する
2. `GET /api/auth/microsoft/link-redirect`(認証必須)がMicrosoftログインURLを返す。
   `state`にログイン中ユーザーIDを暗号化して載せる(OAuthリダイレクトを挟むため、
...
- 対象ユーザーが既に別のMicrosoft 365アカウントと連携済み

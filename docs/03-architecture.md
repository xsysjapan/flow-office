...
## 3.8 承認は状態遷移の橋渡しであり、承認という行為自体は取り消さない

承認(`*.approved`イベント)は「未確定→確定」等のステータス遷移を引き起こすトリガーに
過ぎない。承認そのものを取り消す・書き換える操作は設けない。状態を巻き戻したい場合は、
「承認取消」ではなく差戻し・取消・巻戻しといった別の専用Command/Eventとして新しい状態
遷移を`stored_events`に追記する(3.2節参照)。既存の承認イベントは削除・書き換えの対象と
しない。

締め後(ロック後)の状態を戻す操作は権限を限定する(例: 管理者・バックオフィス担当者のみ)。
申請者自身の通常操作としては提供しない(`CancelPaidLeaveRequestHandler`の
`isAdminAction`分岐、`docs/09-usecases-paid-leave.md`「実装上のポイント」参照)。
...
## 3.9 「申請(ワークフロー)」と「業務」を分離する

「申請」(誰が・何を・いつ申請し、誰が承認するかという進行状況)はドメインを横断して
統合的に扱う仕組み(`workflow_requests`)として設計する。一方、各ドメイン固有の
未確定ステート・金額計算・残高計算・確定処理などの「業務」ロジックは、統合ワークフロー
側に混入させず専用ドメイン(`App\Domain\PaidLeave`/`CompensatoryLeave`/`ShiftSwap`/
`Expense`等)に留める。両者は`workflow_requests.subject_type`/`subject_id`のような
ポリモーフィック関連で連携するに留め、統合ワークフロー側のテーブル・イベントに業務固有の
詳細な計算結果を持たせない(docs/10-usecases-workflow.md「実装上のポイント」の
振替休日申請・代休申請の実装例を参照)。

業務によっては、承認ワークフローを経由せず直接処理される「申請不要」のケースがある
(例: 代休の「付与」は勤怠実績から自動導出されるため申請不要だが、「消化」は申請制、

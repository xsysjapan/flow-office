...
18. **ユーザー・グループ・アクセス管理基盤**: 管理者が「システム」のグループ種別画面、
    ...
    所属変更の作成フォームが「変更予約作成」ボタンからモーダルで開くこと、下書き・予約済み行の「変更」から
    現在値を引き継いだ編集モーダルが開き、変更内容を保存できることを確認する。
    グループコードを画面へ表示せず内部生成すること、ユーザー詳細の所属追加・解除・移動・主所属変更で
    即時反映と日時指定予約を選択できること、ユーザー詳細・グループ詳細の変更履歴で操作とグループ名を
    日時より一段目立たせ、状態を日本語名称で表示すること、予約を取り消せることも確認する。
    ユーザー一覧のグループ種別・状態も日本語名称で表示する。
    「システム」のID・管理元設定画面ではExternalIdentity、FieldAuthority、認証キーを管理する。
    さらに、独立したアクセス管理画面の各タブからGroupへ付与したFeatureとRole/Permissionが所属ユーザーの
    有効アクセスへ即時反映され、
...
19. **承認者=申請者本人のスキップ承認・承認不要設定時の承認者欄非表示**: 汎用申請・有給・
    特別休暇・月次勤怠のそれぞれで、承認者に申請者本人を指定して提出すると、承認待ちを
    経由せず提出と同時に「承認済み」まで確定すること(承認ルート自体はあるが実質的な
    承認者がいないケースであり、「承認不要」システム設定とは別の仕組み)。また、
    「承認不要」にシステム設定されているドメイン(特別休暇で確認する)では、申請フォームに
    承認者選択欄自体を表示しないこと(`frontend/e2e/scenario-99-additional.spec.ts`の
    §5-19)。

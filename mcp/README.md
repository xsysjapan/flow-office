...
勤怠操作用ツールをアプリのルート(`/`、MCPのJSON-RPC 2.0、Streamable HTTP)で提供する。
...
mcp/ (このアプリ。独自DB: oauth_*, mcp_users, mcp_user_backend_tokens)
  │ 4. MCPツール呼び出し: POST / (Bearer: mcp/発行のOAuthアクセストークン)
...
- Claudeに対しては**OAuth2認可サーバー + リソースサーバー**(`/oauth/*`, JSON-RPCは`/`)。
...
Caddy・ngrokは**devcontainerの中ではなくホスト側(Mac)のターミナル**で起動する
(`docker compose`がbackend/frontend/mcpの各ポートをすでに`127.0.0.1`へ公開しているため、
ホスト側のCaddyから直接届く)。
...
以下を実行すると、ngrokトンネルを張ったうえで、割り当てられた公開URLを環境変数として
`docker compose up --build`にそのまま渡して起動する。backend・frontend・mcpは最初から
`/flow-office`配下の公開URL向け設定で立ち上がるため、以前のように起動後にfrontend/mcpを
手動で再起動する必要はない。
...
MCPクライアントへの登録URLは `https://xxxx.ngrok-free.app/flow-office/mcp` になる。
mcpのJSON-RPCエンドポイントの実体はアプリのルート(`/`)なので、マウントパスそのものが
そのままエンドポイントになり、`resource`/`issuer`メタデータもこのURLと一致する
(`scripts/tunnel/Caddyfile`参照)。

Ctrl+Cで`docker compose`・ngrok・Caddyをまとめて終了する。
...
ハッピーパスとスコープ不足時の拒否を、`tests/Feature/Mcp/` がルート(`/`)のtools/list・
tools/call(成功・backend APIエラーの`isError`整形・OAuthスコープ不足時の拒否)を検証する
(いずれも`Http::fake()`でbackendをモックする)。

実際に動いている`backend/`に対する手動確認は、`backend/`と`mcp/`の両方を起動したうえで、
DCR → 認可(PKCE) → トークン交換 → `/`の順にcurlで叩くことで行える(手順は

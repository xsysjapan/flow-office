...
トークン節約のため、「調査」と「実装作業」の両方を積極的にサブエージェント
(Agentツール)に委譲する。メインの会話コンテキストにはソースコード全文・diff・
調査ログを流し込まず、要約・完了報告だけを戻させる。

- 作業対象のディレクトリ(`backend/`・`frontend/`・`mcp/`)の`CLAUDE.md`をまず読み、
  他のディレクトリの実装詳細は必要になるまで読まない。
- ファイルの場所が分かっている場合はGrep/Globで直接開く。「関連しそうなファイルを広く
  読む」探索は極力避け、対象ドメイン・機能名のディレクトリ1つに絞る。
- 複数ドメイン・複数ディレクトリにまたがる調査や、該当ファイルが特定できない広い探索は
  Exploreサブエージェントに委譲し、メインの会話コンテキストを消費させない。
- **`.claude/skills/`のスキルに沿って完結する定型実装作業**(例:
  `add-frontend-component`でコンポーネント+story+testの3点セットを1つ作る、
  `add-api-hook`で型・クライアント関数・hookを1本追加する、`add-domain-event`で
  Command/Event/Handlerを1セット追加する等)は、実装そのものを
  general-purposeサブエージェントに一任する。委譲時は対象ドメイン名・参照すべき
  既存実装ファイルパス・従うべきスキル名を明示し、探索や設計判断は丸投げしない
  (何を作るかはこちらで決め、どう書くかを委譲する)。
- 独立した複数ファイル・複数コンポーネントを追加/修正する場合、メインの会話で
  1つずつ順に作業するのではなく、依存関係がないものはAgentを並列起動して同時に
  進める。

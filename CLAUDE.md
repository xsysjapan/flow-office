# flow-office

汎用勤怠・申請・バックオフィス処理システム。バックエンド(Laravel + MySQL + XSERVER)と
フロントエンド(Vite + React + TypeScript)を分離したモノレポ構成。

## リポジトリ構成

```
backend/     Laravel API (Sanctumトークン認証)。実装の詳細は backend/CLAUDE.md を参照
frontend/    Vite + React + TypeScript のSPA。Storybook導入済み
mcp/         ClaudeなどのAIアプリへ勤怠操作ツールを提供するMCPサーバー。backend/とは別の
             Laravelアプリ(独立したcomposer.json・DB)として実装し、backend/のDBには
             アクセスしない。/mcp をエンドポイントとし、認可はOAuth2(Dynamic Client
             Registration対応)で行う(docs/25-usecases-integrations-mcp.md)。詳細は
             mcp/README.md を参照
docs/        設計ドキュメント(全体の目次は docs/README.md)
.claude/     開発でよく使うパターンをまとめたスキル
```

バックエンドとフロントエンドは別々にデプロイされることを前提にする。認証はSanctumの
Bearerトークン方式(Cookieベースではない)。

`backend/`・`frontend/`・`mcp/` にはそれぞれ`CLAUDE.md`があり、セットアップ手順・
ディレクトリ構成・スキル一覧はそちらを参照する(このファイルには重複して書かない)。
作業対象のディレクトリに応じて該当ファイルだけを読む。

## 効率的なコード参照とサブエージェントへの委譲

トークン節約のため、「調査」と「実装作業」の両方を積極的にサブエージェント
(Agentツール)に委譲する。メインの会話コンテキストにはソースコード全文・diff・
調査ログを流し込まず、要約・完了報告だけを戻させる。

- 作業対象のディレクトリ(`backend/`・`frontend/`・`mcp/`)の`CLAUDE.md`をまず読み、
  他のディレクトリの実装詳細は必要になるまで読まない。
- ファイルの場所が分かっている場合はGrep/Globで直接開く。「関連しそうなファイルを広く
  読む」探索は極力避け、対象ドメイン・機能名のディレクトリ1つに絞る。
- 複数ドメイン・複数ディレクトリにまたがる調査や、該当ファイルが特定できない広い探索は
  `.claude/agents/investigator.md`(安価なモデルで動作する読み取り専用エージェント)に
  委譲し、メインの会話コンテキストを消費させない。単一ファイル・単一シンボルの特定など
  ごく軽い探索はExploreサブエージェントで足りる。
- **`.claude/skills/`のスキルに沿って完結する定型実装作業**(例:
  `add-frontend-component`でコンポーネント+story+testの3点セットを1つ作る、
  `add-api-hook`で型・クライアント関数・hookを1本追加する、`add-domain-event`で
  Command/Event/Handlerを1セット追加する等)や、変更セットの「仕様確定事項」に
  基づく実装は、`.claude/agents/implementer.md`(安価なモデルで動作する実装専任
  エージェント)に一任する。仕様検討・タスク分解(何を作るか)はメインの会話
  (現行モデル)が担い、コーディングそのもの(どう書くか)だけを委譲する。委譲時は
  対象ドメイン名・参照すべき既存実装ファイルパス・従うべきスキル名を明示し、
  探索や設計判断は丸投げしない
- **委譲したエージェントの完了レビューを徹底する**: レビューはinvestigator/implementer
  等の安価なモデルに任せず、必ずそれらより上位のモデル(メインの会話。委譲元自身が
  安価なモデルの場合はさらに上位のモデル)が行う。エージェントの報告を鵜呑みにせず、
  当初依頼した目標に対して実際に達成できているか(該当ファイルが存在するか、
  テストが本当に通っているか等)を都度確認する。未達成・部分的な失敗があった場合は
  簡単に振り返りを行い、原因が「委譲時の指示不足」「スキル記述の曖昧さ・漏れ」など
  簡単に直せるものであれば、該当する`.claude/skills/`のSKILL.mdや
  `.claude/agents/`のエージェント定義をその場で更新し、次回以降に同じ失敗を
  繰り返させない。原因の切り分けが難しい・仕様判断が絡む場合はユーザーに報告し、
  対処方針を確認する。**このレビュー結果は記憶に留めず、`.claude/skills/
  delegation-review`に従って`docs/delegation-reviews/`へ都度記録する**(委譲元・
  使用モデル・達成状況・レビュー結果・是正内容を残し、後から状況を追跡できるように
  する)。
- **作業の区切りごとにタスク割り当て自体の妥当性も振り返る**: 個々のエージェントの
  成否(完了レビュー)とは別に、一連の開発作業がひと区切りついたタイミングで、
  「仕様検討・タスク分解を上位モデル(メインの会話)が担い、定型的な調査・実装を
  investigator/implementer等の安価なモデルへ委譲できていたか」を振り返る。
  本来委譲できた定型作業をメインの会話が直接処理していないか、逆に設計判断や
  仕様確認が必要な作業を安価なモデルに丸投げしていないかを確認し、割り当てが
  ずれていた場合は次の作業から是正する。振り返りの結果、原因がスキル記述や
  エージェント定義の曖昧さにある場合は、上の完了レビューのルールと同様にその場で
  該当ファイルを更新する。この振り返りも同じく`.claude/skills/delegation-review`
  に従って`docs/delegation-reviews/`へ記録し、後から一覧・評価できるようにする。
- 独立した複数ファイル・複数コンポーネントを追加/修正する場合、メインの会話で
  1つずつ順に作業するのではなく、依存関係がないものはAgentを並列起動して同時に
  進める。
- 大規模な調査・レビュー・移行など、明確に「サブエージェントで並行処理してほしい」
  という指示がユーザーから出た場合のみWorkflowツールでの多段オーケストレーションを
  検討する(通常のタスクでは使わない)。
- 数ファイル以上の実装・調査タスク(新しいAPI/画面/イベントの追加など)に着手する前に、
  `.claude/skills/changeset`に沿って、まず`docs/changesets/YYYYMMDD-変更名/`配下に
  変更セット(仕様検討ドキュメント。モック等のアセット含む)を作成し、そこで仕様検討の
  各決定事項(選択肢・決定・理由・対象外・ドキュメントへの影響)を曖昧さなく詰め切ってから
  実装に入る(単純な1行修正や質問には使わない)。ユーザーレビューを経ずにいきなり
  コードを書き始めない。

### トークン使用量を抑える

- 会話が長引いたら`/compact`(要約して継続)や`/clear`(無関係な次のタスクに移る際に
  破棄)を使い、肥大化したら`/context`で内訳を確認する。
- ソース全文・diff・ログ全文をメインの会話に貼らない。サブエージェントには要約のみを
  戻させ、詳細はサブエージェント側のコンテキストに留める。
- ファイルを広く`cat`的に読まず、Grep/Globで対象箇所だけを開く(このファイルの
  「効率的なコード参照」も参照)。ただし、探索を減らそうとして必要な確認を省略し、
  誤った変更→手戻りで結果的に読み直しが増えることの方が高くつく。範囲を絞ることと、
  対象範囲内をきちんと確認することは両立させる。

## 絶対に外してはいけない設計原則

1. **EventStoreを正とする**: 状態変更は原則として Command → CommandHandler → `stored_events` の
   流れで行う。例外は個別仕様で明示する。`system_settings`は管理者専用APIから直接更新するが、
   監査用イベントを同一トランザクションで`stored_events`へ記録する。
2. **Projectionは再生成可能な派生データ**: 画面表示用テーブルはイベントから再生成できる
   前提で設計する。Projectionを直接手で書き換えない。
3. **勤怠の正は日次実績・勤務予定・有給付与**: `employee_shift_assignments` /
   `attendance_days` / `attendance_breaks` / `paid_leave_grants` が正。入力・編集は必ず
   日次単位で行わせ、`attendance_months`はその日次実績を集計・確定した結果であり、
   月次側の項目を直接入力させない。
4. **週次勤怠は日次勤怠の編集ビュー**: 独立データとして持たない。保存時は日次単位の
   編集イベントに分解する。
5. **承認者は都度指定**: 固定承認ルートに加え、申請時に任意の社員を承認者に指定できる
   ようにする。
6. **バックオフィス処理は承認とは別ステータス系列**で管理する。
7. **Teamsは通知専用**。チャット・掲示板・お知らせ機能は作らない。
8. **法務判断が必要な値はマスタ化する**(有給付与ルール、残業計算ルールなど)。
   ハードコードしない。最終設定は社労士確認が前提。
9. **操作経路と業務ロジックを分離する**: Web・モバイル・共有Android打刻リーダー・個人端末・
   外部端末・API・MCPのどこから操作されても、最終的には共通のCommand→CommandHandlerへ
   集約する。入口ごとに勤怠計算ロジックを複製しない(docs/03-architecture.md 3.5)。
10. **打刻と勤怠編集を区別する**: 打刻(`attendance_punches`への追記のみ、削除・上書きしない)
    と、勤怠編集(`attendance_days`の作成・修正)は別の操作として扱う(docs/03-architecture.md 3.6)。
11. **AIは勤怠ルールを決定しない**: Claude等のAIアプリ・MCPサーバーは読解・下書き生成・
    対話に限定し、労働時間計算・休日判定・休暇残高・締め判定・承認ルール等は必ず勤怠管理API
    側で行う。AI推定値はユーザー確認前に正データとして確定させない(docs/03-architecture.md 3.7)。
12. **端末・認証キー・アプリ連携は別概念として扱う**: 打刻専用の端末エンティティを作らず、
    端末(`devices`)・認証キー(`authentication_keys`)・API/MCP連携(`application_integrations`)
    を独立したドメインとして設計する。生体情報(指紋画像・顔画像・特徴量)は保存せず、外部
    認証端末が発行した利用者IDと認証結果のみを保存する(docs/23〜docs/25参照)。
13. **承認は状態遷移の橋渡しに過ぎず、取り消し不可**: 承認という行為自体を取り消す操作は
    設けない。状態を巻き戻す場合は「承認取消」ではなく、差戻し・取消などの別の専用
    Command/Eventとして新しい状態遷移を`stored_events`に追記する(既存の承認イベントは
    削除・書き換えない)。締め後(ロック後)の巻戻しは管理者・バックオフィス担当者等に
    権限を限定し、申請者自身には行わせない。
14. **「申請(ワークフロー)」と「業務」は分離する**: 誰が・何を・いつ申請し誰が承認するか
    という進行状況(`workflow_requests`)はドメイン横断で統合的に扱うが、各ドメイン固有の
    未確定ステート・金額計算・残高計算・確定処理などの業務ロジックは統合ワークフロー側に
    混入させず、専用ドメイン(`App\Domain\PaidLeave`/`CompensatoryLeave`/`ShiftSwap`/
    `Expense`等)側に留める。両者は`subject_type`/`subject_id`のポリモーフィック関連で
    連携するに留める。また、代休の付与のように業務によっては承認ワークフローを経由せず
    直接処理される「申請不要」のケースがあるため、統合ワークフローの仕組みは申請が実際に
    発生したケースのみを扱うものとして設計し、申請不要で処理される業務データを無理に
    統合ワークフロー側に取り込まない(docs/03-architecture.md 3.9、
    docs/10-usecases-workflow.md参照)。

これらはバックエンドAPIの設計原則。詳細は `docs/03-architecture.md` と
`docs/20-implementation-notes.md` を参照。

## ドキュメント構成

`docs/README.md` の目次を参照。ユースケースは `docs/06`〜`docs/15` に、テーブル定義は
`docs/16-database-schema.md`、イベント一覧は `docs/17-events.md` にある。
